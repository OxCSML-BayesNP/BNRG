...
if args.data is None:
    w_true = sample_w(args.nu, args.a, args.b, args.n)
    s_true = np.sqrt(args.C*npr.dirichlet(5*np.ones(args.C)))
    r_true = np.sqrt(args.C)
...
if args.n_init > 0:
    init_chain, w_init = gignrg.mcmc.run_mcmc(graph, args.n_init, return_w=True)
    nu_init = init_chain['nu'][-1]
    a_init = init_chain['a'][-1]
    b_init = init_chain['b'][-1]
else:
    w_init = None
    nu_init = None
    a_init = None
...
for i in range(args.n_chains):
    ...
    chains[i] = run_mcmc(graph, args.C, args.n_samples,
            ...
            w=w_init, nu=nu_init, a=a_init, b=b_init)
    print
...
for i in range(args.n_chains):
    results['nu'] += chains[i]['nu']
    results['a'] += chains[i]['a']
    results['b'] += chains[i]['b']
...
for i in range(n_post_samples):
    ...
    w = sample_w(results['nu'][i], results['a'][i], results['b'][i], graph['n'])
